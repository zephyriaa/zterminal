...
function runEmaCross(bars: Bar[]) {
  if (bars.length < 60) return [];
  const closes = bars.map((bar) => bar.c); const fast = ema(closes, 20); const slow = ema(closes, 50);
...
function summarize(returns: number[], totalBars: number): Result {
...
export function BacktesterView() {
  ...
  async function loadAndRun() {
    ...
    try {
      ...
      const nextBars = payload.bars as Bar[]; const returns = runEmaCross(nextBars);
      if (returns.length < 10) throw new Error("The verified window does not contain enough EMA-cross trades for validation.");
      setBars(nextBars); setResult(summarize(returns, nextBars.length)); setState("ready");
