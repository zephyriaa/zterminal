...
type Result = {
  ...
  walkForwardResults: { index: number; inSampleReturn: number; oosReturn: number; oosTrades: number }[];
  rMultiple: { mean: number; median: number; worst: number; riskFraction: number };
};
...
function runEmaCross(bars: Bar[], fastPeriod = 20, slowPeriod = 50, fromIndex?: number, toIndex = bars.length) {
  const warmup = Math.max(fastPeriod, slowPeriod);
  const start = Math.max(warmup, fromIndex ?? warmup); const end = Math.min(toIndex, bars.length);
  if (end <= start) return [];
  ...
  if (position !== 0 && entry > 0) returns.push((((bars[end - 1]?.c ?? entry) - entry) / entry) * position);
  return returns.filter(Number.isFinite);
}

function summarize(returns: number[], totalBars: number, fast: number, slow: number, sensitivity: Result["sensitivity"], walkForwardResults: Result["walkForwardResults"], riskFraction: number): Result {
  ...
  const rValues = returns.map((value) => value / riskFraction).sort((left, right) => left - right);
  const middle = Math.floor(rValues.length / 2); const median = rValues.length % 2 ? rValues[middle] : (rValues[middle - 1] + rValues[middle]) / 2;
  return { trades: returns.length, netReturn: equity - 1, winRate: returns.length ? wins.length / returns.length : 0, profitFactor: grossLoss ? grossWin / grossLoss : wins.length ? Infinity : 0, maxDrawdown, bootstrap: bootstrapMean(returns, { samples: 500, confidenceLevel: 0.95, seed: 42 }), monteCarlo: simulateTradeSequence(returns, { paths: 500, initialEquity: 1, seed: 42 }), walkForward: walkForwardResults.length, fast, slow, sensitivity, walkForwardResults, rMultiple: { mean: rValues.reduce((sum, value) => sum + value, 0) / rValues.length, median, worst: rValues[0], riskFraction } };
}

export function BacktesterView() {
  const { symbol, timeframe, connection } = useWorkspace();
  const [bars, setBars] = useState<Bar[]>([]); const [state, setState] = useState<RunState>("idle"); const [error, setError] = useState<string | null>(null); const [result, setResult] = useState<Result | null>(null); const [fast, setFast] = useState("20"); const [slow, setSlow] = useState("50"); const [riskPct, setRiskPct] = useState("1");
  ...
  async function loadAndRun() {
    ...
    try {
      ...
      const walkForwardResults = createWalkForwardWindows(nextBars.length, { inSample: 300, outOfSample: 100, step: 100, purge: 1 }).map((window) => {
        const inSample = runEmaCross(nextBars, fastPeriod, slowPeriod, window.inSample.from, window.inSample.to); const oos = runEmaCross(nextBars, fastPeriod, slowPeriod, window.outOfSample.from, window.outOfSample.to);
        return { index: window.index + 1, inSampleReturn: compoundReturns(inSample), oosReturn: compoundReturns(oos), oosTrades: oos.length };
      });
      setBars(nextBars); setResult(summarize(returns, nextBars.length, fastPeriod, slowPeriod, sensitivity, walkForwardResults, riskFraction)); setState("ready");
...
function compoundReturns(values: number[]) { return values.reduce((equity, value) => equity * (1 + value), 1) - 1; }
