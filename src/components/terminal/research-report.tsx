...
export function ResearchReport() {
  ...
  return <div className="zt-research-report">
    ...
    {busy && <div className="zt-report-run-status" role="status">{job!.stage.replaceAll("_", " ")} <button onClick={() => useResearch.getState().cancel()}>Cancel run</button><span>First engine compilation can take about a minute. Downloads and initialization are separate from warm execution.</span></div>}
...
function Provenance({ result }: { result: ResearchResult }) {
  return <dl className="zt-provenance"><dt>Source SHA-256</dt><dd>{result.sourceHash}</dd><dt>Dataset SHA-256</dt><dd>{result.dataset.hash}</dd><dt>Result SHA-256</dt><dd>{result.resultHash}</dd><dt>Engine</dt><dd>Python {result.engine.python} / vectorbt {result.engine.vectorbt} / SDK {result.engine.sdk} / analytics {result.engine.analytics}</dd><dt>Saved locally</dt><dd>{date(result.createdAt)} UTC · {result.dataset.bars.length.toLocaleString()} complete candles</dd></dl>;
}
function SeriesPlot({ label, series }: { label: string; series: { name: string; values: number[]; color: string }[] }) {
...
function Histogram({ label, values }: { label: string; values: number[] }) {
  ...
  const min = Math.min(...values), max = Math.max(...values), span = max - min || 1, bins = Array(20).fill(0) as number[];
