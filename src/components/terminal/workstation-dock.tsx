...
const DEFAULT_RESEARCH_STAGES = [
  { label: "Hypothesis", detail: "Add a documented research hypothesis", state: "active" },
  { label: "Data", detail: "Attach a verified provider manifest", state: "pending" },
  { label: "Chart evidence", detail: "Record evidence from the selected window", state: "pending" },
  { label: "Model", detail: "Declare explicit strategy rules", state: "pending" },
  ...
  const runBacktest = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/research/jobs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ schema_version: "research.v2.0", kind: "strategy_backtest", artifact_id: lastCompile.artifactId, dataset_manifest: { provider: connection.provider, native_symbol: symbol, timeframe, from_ms: now - 30 * 86_400_000, to_ms: now, quality_status: connection.dataStatus }, execution_policy: { fill_model: "next_bar_open", commission_per_contract: 0, slippage_ticks: 0, spread_ticks: 0, position_size: 1 } }),
      });
      const result = await response.json();
...
function ResearchPanel({ hasResult }: { hasResult: boolean }) {
  ...
  return <div className="h-full p-3 overflow-y-auto scroll-thin"><div className="flex items-start justify-between"><div><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Research notebook</div><div className="text-[12px] mt-1">No hypothesis recorded in this session</div></div><button className="dock-action" disabled><Save className="w-3 h-3" />Save note</button></div><div className="research-stages mt-4 grid grid-cols-6 gap-1.5">{stages.map((stage, index) => <div key={stage.label} className="relative"><div className={cn("h-1 mb-2", stage.state === "active" ? "bg-mdata" : stage.state === "ready" ? "bg-pos/70" : "bg-foreground/15")} /><div className="text-[9px] uppercase tracking-[0.12em] text-muted-foreground">0{index + 1} · {stage.label}</div><div className="mt-1 text-[10px] text-foreground/80 leading-snug">{stage.detail}</div></div>)}</div><div className="research-insights mt-5 grid grid-cols-[1fr_1fr] gap-3"><div className="border hairline bg-surface/40 p-2.5"><div className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Observation</div><div className="mt-2 text-[10.5px] leading-relaxed text-muted-foreground">No observation has been saved. Add evidence from the chart or research workspace before making a claim.</div></div><div className="border hairline bg-surface/40 p-2.5"><div className="text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Next action</div><div className="mt-2 text-[10.5px] leading-relaxed text-foreground/80">{hasResult ? "Inspect the verified result, then compare it against an out-of-sample window." : "Write and validate explicit strategy rules before requesting a research run."}</div></div></div></div>;
}

function DataPanel({ symbol, timeframe, provider, dataStatus }: { symbol: string; timeframe: string; provider: string; dataStatus: string }) {
  return <div className="data-panel-grid h-full grid grid-cols-[1fr_1fr] divide-x divide-[color-mix(in_oklch,var(--foreground)_8%,transparent)]"><div className="p-3"><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Data contract</div><div className="mt-3 space-y-2">{[["Provider", provider.toUpperCase()], ["Native symbol", symbol], ["Timeframe", timeframe], ["Feed state", dataStatus], ["Execution", "Next-bar open"]].map(([label, value]) => <div key={label} className="flex items-center justify-between border-b hairline pb-1.5 text-[10.5px]"><span className="text-muted-foreground">{label}</span><span className="font-mono-num text-foreground">{value}</span></div>)}</div></div><div className="p-3"><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Evidence rules</div><div className="mt-3 space-y-2 text-[10.5px] text-foreground/80 leading-relaxed"><div className="flex gap-2"><CheckCircle2 className="w-3.5 h-3.5 text-pos shrink-0" />Source and coverage stay visible beside the chart.</div><div className="flex gap-2"><CheckCircle2 className="w-3.5 h-3.5 text-pos shrink-0" />Stale or unavailable data is withheld.</div><div className="flex gap-2"><Radio className="w-3.5 h-3.5 text-warn shrink-0" />Public-market research only; execution is disabled.</div></div></div></div>;
}

function AlertsPanel({ search, setSearch }: { search: string; setSearch: (value: string) => void }) {
  return <div className="h-full p-3"><div className="flex items-center gap-2"><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Alert ledger</div><div className="ml-auto relative"><Search className="absolute left-2 top-1.5 w-3 h-3 text-muted-foreground" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter saved alerts" className="h-6 w-36 bg-surface border hairline pl-6 pr-2 text-[10px] outline-none focus:border-mdata/50" /></div></div><div className="mt-3 border hairline bg-surface/40 p-3 text-[10.5px] leading-5 text-muted-foreground">No persisted alerts are available in this workspace. Create session alerts from the Alerts view; provider evaluation and durable notifications remain unavailable until their services are connected.</div></div>;
}
