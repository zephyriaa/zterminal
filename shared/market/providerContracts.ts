...
export const PROVIDER_CATALOG: readonly ProviderCatalogEntry[] = [
  {
    ...
    capabilities: [
      ...
      { capability: "PUBLIC_TRADES", state: "AVAILABLE", reason: "Bounded public trade stream preserves exchange-reported signed taker size with stale, reconnect, and idle-expiry states." },
      ...
      { capability: "ORDER_BOOK_DEPTH", state: "AVAILABLE", reason: "REST snapshot plus sequenced WebSocket delta reconciliation is released with explicit syncing, stale, degraded, and unavailable states." },
    ...
    notice: "Active public read-only provider. Released order-flow capabilities remain explicitly bounded and live-only; unavailable states are rendered rather than backfilled.",
  },
  {
    id: "binance_usdm",
    ...
    environments: ["PRODUCTION", "TESTNET"],
    capabilities: [
      { capability: "SNAPSHOT", state: "VERIFYING", reason: "A public ticker adapter is not yet released in the canonical runtime." },
      { capability: "HISTORICAL_CANDLES", state: "VERIFYING", reason: "A bounded historical adapter is not yet released in the canonical runtime." },
      { capability: "CONTRACT_METADATA", state: "VERIFYING", reason: "Contract metadata adapter is not yet released in the canonical runtime." },
      { capability: "PUBLIC_TRADES", state: "VERIFYING", reason: "Adapter and deterministic semantic fixtures are implemented, but the bounded release-environment probe did not observe a live WebSocket event; the UI exposes a degraded state rather than claiming availability." },
      { capability: "BEST_BID_OFFER", state: "VERIFYING", reason: "No canonical BBO adapter is released." },
      { capability: "ORDER_BOOK_DEPTH", state: "VERIFYING", reason: "Depth snapshot and sequence reconciliation is not released; depth is not rendered from this venue." },
      { capability: "CANDLE_STREAM", state: "UNAVAILABLE", reason: "No canonical public candle stream is released." },
      { capability: "OPTIONS_CHAIN", state: "UNAVAILABLE", reason: "USDⓈ-M perpetuals are not an options source." },
      { capability: "GREEKS", state: "UNAVAILABLE", reason: "USDⓈ-M perpetuals do not provide options Greeks." },
      { capability: "OPEN_INTEREST_BY_STRIKE", state: "UNAVAILABLE", reason: "USDⓈ-M perpetuals do not provide options open interest by strike." },
    ],
    notice: "Public-read-only adapter under live transport verification. It does not provide a released depth, candle, or durable-history contract in ZTerminal.",
  },
  {
    id: "bybit_linear",
    ...
    capabilities: [
      ...
      { capability: "HISTORICAL_CANDLES", state: "VERIFYING", reason: "A bounded historical adapter is not yet released in the canonical runtime." },
      { capability: "CONTRACT_METADATA", state: "VERIFYING", reason: "Contract metadata adapter is not yet released in the canonical runtime." },
      { capability: "PUBLIC_TRADES", state: "AVAILABLE", reason: "Bounded public trade stream preserves Bybit’s reported taker side with explicit stale and reconnect states." },
      { capability: "BEST_BID_OFFER", state: "VERIFYING", reason: "No canonical BBO adapter is released." },
      { capability: "ORDER_BOOK_DEPTH", state: "VERIFYING", reason: "Bybit snapshot/delta depth semantics are documented but not yet released in the canonical runtime." },
      { capability: "CANDLE_STREAM", state: "UNAVAILABLE", reason: "No canonical public candle stream is released." },
      { capability: "OPTIONS_CHAIN", state: "UNAVAILABLE", reason: "Linear perpetuals are not an options source." },
      { capability: "GREEKS", state: "UNAVAILABLE", reason: "Linear perpetuals do not provide options Greeks." },
