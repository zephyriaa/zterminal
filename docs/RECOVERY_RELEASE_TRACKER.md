...
| CWP1 | Account-isolated cloud workspace and installable web app | Versioned terminal-preference sync into an authenticated TiDB workspace with explicit conflict resolution, guest local fallback, responsive mobile terminal, manifest, icons, and conservative service-worker packaging | **Complete in production** (`155914a`; deployed in `dep-da3fqfgu01pc738klvig`) | TiDB schema verification, protected-router ownership tests, 93-test regression, production build, live manifest/service-worker/install-control smoke; no runtime caching of account, workspace, or market data |
...
**Current production state:** MEX1, MEX2, OF1, P15, P14, OF2, IQ1, ACC1, UX2, IND2, and CWP1 are live on `https://zterminal.onrender.com`. The product exposes evidence-backed public-feed health, a bounded multi-exchange tape foundation, browser-local guest persistence plus authenticated account-isolated cloud synchronization, opt-in Flow Pulse, premium one-screen workstation hierarchy, a closed coded-strategy path to deterministic historical-candle evaluation, UTC session candle-volume context, selected-venue live reported-size large-print evidence, native safe indicators, responsive web-app installation support, and keyboard-first interaction quality. Strategy source is not JavaScript and can neither access host capabilities nor create any execution route. Flow Pulse and large-print evidence remain descriptive rather than automated: selected-venue tape and Gate.io depth remain separately labelled, with no cross-venue consolidation, dollar-notional conversion, prediction, or execution claim. Binance remains `VERIFYING`, and both Binance and Bybit were displayed as `DEGRADED` in the production verification; the interface does not claim live status absent current evidence.

**Current controlled boundary:** Coinbase depth, cross-venue Flow Pulse combination, broker credentials, execution routes, and arbitrary user-code execution remain deliberately unavailable. Cloud preferences are account-isolated and limited to bounded terminal interface state; they do not store credentials, live feeds, custom formulas, research protocols, or strategy source.
...
- [Cloud workspace and installable web app validation](./CLOUD_WORKSPACE_PWA_VALIDATION.md)
