...
| IQ1 | Product-sprint interaction quality and accessibility | Keyboard reference, semantic command-palette navigation, focus restoration, live Focus-mode status, and strict chart-only Focus view | **Complete on product branch** (pending commit; not production-promoted) | Command fixtures, desktop browser evidence, static responsive-rule validation, full regression and build |
