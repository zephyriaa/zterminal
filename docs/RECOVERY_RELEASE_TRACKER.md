...
| UX2 | Auth-aware terminal and code-first backtesting | Visible signed-in/guest terminal account control; closed-source strategy compile plus verified candles is sufficient for historical evaluation; optional protocol baseline remains intact | **Approved for production release** (`release/account-indicator-production`) | TypeScript check, 88-test regression, production build, live local guest/catalog/verified-candle browser evidence; no arbitrary code or broker route |
| IND2 | Native indicator catalog | Source-attributed native trend, momentum, volatility, and price-context presets compiled through the existing closed AST runtime | **Approved for production release** (`release/account-indicator-production`) | Catalog compilation/evaluation fixture, safe-runtime vocabulary tests, direct browser add-to-chart evidence, indicator source ledger; no Pine/community-script import |
