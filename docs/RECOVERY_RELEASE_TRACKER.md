...
**Next active product slice: complete the remaining professional order-flow work, starting with transparent session-volume context and configurable large-trade evidence only where a verified source supports it.** MEX1, MEX2, OF1, P15, and P14 are complete on `product/orderflow-research-terminal`. The product now exposes evidence-backed public-feed health, a bounded multi-exchange tape foundation, browser-local interface persistence, opt-in Flow Pulse evidence, a premium one-screen workstation hierarchy, and a closed coded-strategy path to deterministic historical-candle evaluation. Strategy source is not JavaScript and can neither access host capabilities nor create any execution route. Flow Pulse remains descriptive rather than automated: selected-venue tape and Gate.io depth remain separately labelled, with no cross-venue consolidation, prediction, or execution claim. Binance’s adapter and fixtures exist but it remains `VERIFYING`; the UI displays `DEGRADED` rather than claiming a live feed until a release-environment WebSocket event is captured.
