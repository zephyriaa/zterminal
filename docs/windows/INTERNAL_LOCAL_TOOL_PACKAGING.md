...
**Status:** The Windows native CMake build now places three unsigned internal executables in its private build output: `ZTerminalWindowsHost.exe`, `zt-local-scene-bridge.exe`, and `zt-offline-provider-import.exe`. This is an internal engineering package only. It is **not** an installer, public download, signed release, updater, or production distribution channel.
...
The raw record is retained at `docs/windows/benchmarks/windows-offline-import-local-scene-smoke.json`.
