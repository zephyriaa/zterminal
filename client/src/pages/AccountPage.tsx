...
export default function AccountPage() {
  ...
  if (!user) {
    return <main className="account-page"><section className="account-shell account-guest"><div className="account-icon"><UserRound size={24} /></div><span className="account-kicker">Account access</span><h1>Research ownership, configured deliberately.</h1><p>Public charts and research remain available to guests. Sign in with Google to own account-isolated research drafts and explicitly synchronize bounded workspace preferences across devices.</p><div className="account-actions"><GoogleSignInButton className="account-google-sign-in" unavailableLabel="Google sign-in requires deployment configuration" /><Link href="/terminal" className="account-secondary">Continue as guest</Link></div><small><ShieldCheck size={14} /> Google subject-based identity · no password collection · no trading credentials</small></section></main>;
  }
  ...
  return <main className="account-page"><section className="account-shell account-member"><div className="account-member-header"><div className="account-avatar">{(user.name || user.email || "Z").slice(0, 1).toUpperCase()}</div><div><span className="account-kicker">Signed in to ZTerminal</span><h1>{user.name || "Research account"}</h1><p>{user.email || "Identity verified through the configured account provider"}</p></div><button className="account-logout" onClick={() => logout()}><LogOut size={15} /> Sign out</button></div>
    <div className="account-overview-grid"><article><span>Research drafts</span><b>{storageUnavailable ? "—" : draftCount}</b><small>{storageUnavailable ? "Workspace state unavailable" : "Owned by your account"}</small></article><article><span>Indicator Lab</span><b>Local</b><small>Closed indicators remain device-local by design</small></article><article><span>Execution</span><b>Disabled</b><small>No broker or order route</small></article></div>
    <section className={`account-storage ${storageUnavailable ? "unavailable" : "available"}`}><div><CloudOff size={18} /><div><b>{storageUnavailable ? "Cloud workspace unavailable" : "Cloud workspace available"}</b><p>{storageUnavailable ? "Your account workspace could not be reached right now. Browser-local preferences remain available for retry and no market data or credentials are stored in the workspace." : "Your research drafts are associated with your signed-in account, while bounded terminal preferences can be explicitly synchronized from the terminal."}</p></div></div>{storageUnavailable ? <Link href="/terminal" className="account-secondary">Return to terminal</Link> : <Link href="/terminal" className="account-secondary">Open terminal</Link>}</section>
