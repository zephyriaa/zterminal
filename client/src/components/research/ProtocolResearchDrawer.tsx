...
export function ProtocolResearchDrawer({ dataset, bars, dataContext, onBacktestMarkers, onFeedback, onClose }: { dataset: ResearchDatasetContext | null; bars: BacktestBar[]; dataContext?: BacktestRunContext["data"]; onBacktestMarkers: (markers: BacktestMarker[]) => void; onFeedback: (feedback: Feedback) => void; onClose: () => void }) {
  ...
  const runHistoricalEvidence = () => {
    if (!baseline || !dataset || !dataContext || !bars.length) { onFeedback({ kind: "warning", message: "A locked protocol baseline and verified historical chart window are required for evaluation." }); return; }
    if (!compile.data?.ok) { onFeedback({ kind: "warning", message: "Compile the current closed ZS source successfully before running historical evidence. Source is never evaluated as JavaScript." }); return; }
    const protocol = stagedChange ? { kind: "INCREMENTAL" as const, baselineFingerprint: baseline.fingerprint, incrementField: stagedChange.field } : { kind: "BASELINE" as const, baselineFingerprint: baseline.fingerprint };
