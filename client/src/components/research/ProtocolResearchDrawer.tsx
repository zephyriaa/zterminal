...
import { BarChart3, CheckCircle2, CircleAlert, Code2, FileText, FlaskConical, GitBranch, ListOrdered, LoaderCircle, LockKeyhole, Play, Save, Settings2, ShieldCheck, X } from "lucide-react";
...
type Tab = "protocol" | "strategy" | "properties" | "overview" | "trades";
...
const STARTER_STRATEGIES = [
  { id: "ema-cross", label: "EMA crossover", detail: "Long-only EMA crossover using loaded verified candles.", source: `strategy("EMA crossover", overlay=true)\ninput.int("Length", 20, minval=1, maxval=200, step=1)\nvar trend = ema(close, Length)\nif crossover(close, trend)\n  strategy.entry("ema-long", strategy.long, qty=1)\nif crossunder(close, trend)\n  strategy.close("ema-long")` },
  { id: "custom", label: "Custom ZS source", detail: "Edit a closed source directly; it is never evaluated as JavaScript.", source: "" },
] as const;
...
export function ProtocolResearchDrawer({ dataset, bars, dataContext, onBacktestMarkers, onFeedback, onClose, embedded = false }: { dataset: ResearchDatasetContext | null; bars: BacktestBar[]; dataContext?: BacktestRunContext["data"]; onBacktestMarkers: (markers: BacktestMarker[]) => void; onFeedback: (feedback: Feedback) => void; onClose: () => void; embedded?: boolean }) {
  ...
  const [selectedStarter, setSelectedStarter] = useState<(typeof STARTER_STRATEGIES)[number]["id"]>("ema-cross");
  const [strategySource, setStrategySource] = useState<string>(STARTER_STRATEGIES[0].source);
  ...
  useEffect(() => {
    ...
    worker.onmessage = (event: MessageEvent<{ id: string; result?: BacktestResult; error?: string }>) => {
      ...
      if (event.data.result) { setBacktest(event.data.result); setBacktestError(null); setTab("overview"); onFeedback({ kind: "success", message: `Historical evidence ${event.data.result.runId} completed in the dedicated browser worker.` }); }
  ...
  const equitySvgPoints = useMemo(() => {
    const equity = backtest?.equity ?? [];
    if (equity.length < 2) return "";
    const values = equity.map((point) => point.value); const min = Math.min(...values); const max = Math.max(...values); const span = max - min || 1;
    return equity.map((point, index) => `${(index / (equity.length - 1)) * 100},${100 - ((point.value - min) / span) * 100}`).join(" ");
  }, [backtest]);
  const resetBaseline = () => {
  ...
  return <aside className={`research-drawer protocol-research-drawer strategy-tester-drawer ${embedded ? "embedded-strategy-tester" : ""}`} aria-label="Historical Strategy Tester">
    ...
    <div className="protocol-tabs strategy-tester-tabs" role="tablist" aria-label="Strategy Tester tabs">
      ...
      <button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")} role="tab" aria-selected={tab === "overview"}><BarChart3 size={13} /> Overview</button>
      <button className={tab === "trades" ? "active" : ""} onClick={() => setTab("trades")} role="tab" aria-selected={tab === "trades"}><ListOrdered size={13} /> Trades</button>
      <button className={tab === "protocol" ? "active" : ""} onClick={() => setTab("protocol")} role="tab" aria-selected={tab === "protocol"}><FileText size={13} /> Protocol <small>optional</small></button>
    </div>

    {tab === "protocol" && <div className="protocol-content">
    ...
    {tab === "strategy" && <div className="protocol-content strategy-compiler-panel">
      <div className="protocol-banner"><Code2 size={14} /><span><b>Closed ZS strategy source</b><small>Compile a safe strategy, then run only the current verified chart window. Protocol is optional.</small></span><StatusPill tone={compile.data?.ok ? "ready" : "pending"}>{compile.data?.ok ? "READY TO RUN" : "READY TO COMPILE"}</StatusPill></div>
      <section className="protocol-section strategy-builder-section">
        <div className="protocol-section-heading"><span>1</span><div><b>Strategy builder</b><small>Start with a tested closed-source shape or edit a bounded ZS strategy directly.</small></div></div>
        <label className="strategy-template-picker">Starter strategy<select value={selectedStarter} onChange={(event) => { const id = event.target.value as (typeof STARTER_STRATEGIES)[number]["id"]; setSelectedStarter(id); const next = STARTER_STRATEGIES.find((template) => template.id === id); if (next?.source) { compile.reset(); setBacktest(null); setStrategySource(next.source); } }}>{STARTER_STRATEGIES.map((template) => <option value={template.id} key={template.id}>{template.label}</option>)}</select><small>{STARTER_STRATEGIES.find((template) => template.id === selectedStarter)?.detail}</small></label>
        <label>ZS source<textarea className="strategy-source-editor" value={strategySource} onChange={event => { compile.reset(); setSelectedStarter("custom"); setStrategySource(event.target.value); }} maxLength={16_000} spellCheck="false" /></label>
        <div className="strategy-compile-actions"><button className="terminal-secondary-button" onClick={() => compile.mutate({ source: strategySource })} disabled={!strategySource.trim() || compile.isPending}><Code2 size={14} /> {compile.isPending ? "Compiling" : "Compile"}</button><button className="terminal-primary-button" onClick={runHistoricalEvidence} disabled={backtestRunning || !bars.length || !compile.data?.ok}>{backtestRunning ? <><LoaderCircle size={14} /> Running historical test</> : <><Play size={14} /> Run on verified window</>}</button><small>{bars.length.toLocaleString("en-US")} verified bars · signal at close → modeled next-bar-open fill · no execution route</small></div>
      </section>
      {compile.error && <div className="protocol-issues"><span><CircleAlert size={12} />Compiler procedure unavailable: {compile.error.message}</span></div>}
      {compile.data && <section className="protocol-section compile-result-section"><div className="protocol-section-heading"><span>2</span><div><b>{compile.data.ok ? "Validated source" : "Compiler diagnostics"}</b><small>{compile.data.name} · {compile.data.engineVersion} · {compile.data.ok ? "eligible for closed historical interpretation" : "correct errors before evaluation"}</small></div></div><div className="compile-inputs"><b>Discovered inputs</b>{compile.data.inputs.length ? compile.data.inputs.map(input => <span key={input.name}>{input.name} · {input.type} · default {String(input.default)}</span>) : <span>No declared inputs</span>}</div><div className="compile-diagnostics">{compile.data.diagnostics.map((diagnostic, index) => <div className={diagnostic.severity} key={`${diagnostic.line}-${diagnostic.col}-${index}`}><b>{diagnostic.severity.toUpperCase()}</b><span>{diagnostic.line ? `L${diagnostic.line}: ` : ""}{diagnostic.message}</span></div>)}</div><div className="compiler-boundary"><ShieldCheck size={13} /><span>Validated source is interpreted only as a closed AST over verified historical candles. It is never JavaScript, imported, fetched, persisted, or routed to a broker.</span></div></section>}
    </div>}
    {tab === "overview" && <div className="protocol-content strategy-overview-panel">{!backtest ? <div className="strategy-empty-state"><BarChart3 size={22} /><b>No historical run yet</b><p>Compile the closed ZS source, set declared properties, then run the currently verified chart window. This remains a historical research result, not a forecast or trade signal.</p><button className="terminal-primary-button" onClick={() => setTab("strategy")}><Code2 size={14} /> Open strategy</button></div> : <><div className="protocol-banner"><BarChart3 size={14} /><span><b>Performance summary</b><small>{backtest.strategy.label} · {backtest.data.barCount.toLocaleString("en-US")} normalized bars · {backtest.provenance.coverageComplete ? "complete selected coverage" : "coverage not complete"}</small></span><StatusPill tone={backtest.status === "COMPLETED" ? "ready" : "blocked"}>{backtest.status}</StatusPill></div>{backtest.metrics ? <><section className="strategy-equity-card"><header><span>Equity</span><b>{backtest.metrics.finalEquity.toFixed(2)}</b><small>Starting capital {backtest.config.initialCapital.toFixed(2)} · modeled next-bar-open fills</small></header><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Historical equity curve"><polyline points={equitySvgPoints} fill="none" stroke={backtest.metrics.netPnl >= 0 ? "#26a69a" : "#ef5350"} strokeWidth="2.2" vectorEffect="non-scaling-stroke" /></svg></section><div className="tester-metrics-grid"><div><span>Net P&amp;L</span><b className={backtest.metrics.netPnl >= 0 ? "positive" : "negative"}>{backtest.metrics.netPnl >= 0 ? "+" : ""}{backtest.metrics.netPnl.toFixed(2)}</b></div><div><span>Total return</span><b className={backtest.metrics.returnPct >= 0 ? "positive" : "negative"}>{backtest.metrics.returnPct >= 0 ? "+" : ""}{backtest.metrics.returnPct.toFixed(2)}%</b></div><div><span>Max drawdown</span><b>{backtest.metrics.maxDrawdownPct.toFixed(2)}%</b></div><div><span>Win rate</span><b>{backtest.metrics.winRate === null ? "—" : `${(backtest.metrics.winRate * 100).toFixed(1)}%`}</b></div><div><span>Profit factor</span><b>{backtest.metrics.profitFactor === null ? "—" : Number.isFinite(backtest.metrics.profitFactor) ? backtest.metrics.profitFactor.toFixed(2) : "∞"}</b></div><div><span>Trades</span><b>{backtest.metrics.tradeCount}</b></div></div></> : <div className="protocol-blockers"><b>Run did not produce performance metrics</b><span>{backtest.limitations.at(-1)}</span></div>}<section className="tester-evidence-card"><b>Data & execution contract</b><span>{backtest.provenance.provider?.toUpperCase() ?? "PROVIDER UNSPECIFIED"} · {backtest.provenance.symbol ?? "SYMBOL UNSPECIFIED"} · {backtest.provenance.interval ?? "INTERVAL UNSPECIFIED"}</span><small>Signal at close → modeled market fill at next bar open. Commission, spread, slippage, tick size, multiplier, and quantity are retained in the run hash.</small><small>Run {backtest.runId} · {backtest.hash} · no order routing or broker connection.</small></section></>}</div>}
    {tab === "trades" && <div className="protocol-content strategy-trades-panel">{!backtest ? <div className="strategy-empty-state"><ListOrdered size={22} /><b>No trade list yet</b><p>Historical trades appear here after a completed run. They are modeled outcomes, not executable orders.</p></div> : <><div className="protocol-banner"><ListOrdered size={14} /><span><b>List of trades</b><small>{backtest.trades.length.toLocaleString("en-US")} modeled historical trades · costs included</small></span><StatusPill tone={backtest.status === "COMPLETED" ? "ready" : "blocked"}>{backtest.status}</StatusPill></div>{backtest.trades.length ? <div className="tester-trade-table"><div className="tester-trade-head"><span>#</span><span>Entry</span><span>Exit</span><span>Qty</span><span>Net P&amp;L</span></div>{backtest.trades.map((trade) => <div key={trade.id}><b>{trade.id}</b><span>{new Date(trade.entryTime).toISOString().slice(0,16).replace("T"," ")}<small>{trade.entryPrice.toFixed(4)}</small></span><span>{new Date(trade.exitTime).toISOString().slice(0,16).replace("T"," ")}<small>{trade.exitPrice.toFixed(4)} · {trade.reason === "end_of_data_mark" ? "terminal mark" : "next-open exit"}</small></span><span>{trade.quantity}</span><em className={trade.netPnl >= 0 ? "positive" : "negative"}>{trade.netPnl >= 0 ? "+" : ""}{trade.netPnl.toFixed(2)}<small>costs {trade.costs.toFixed(2)}</small></em></div>)}</div> : <div className="protocol-blockers"><b>No closed trades</b><span>{backtest.limitations.at(-1)}</span></div>}<div className="compiler-boundary"><ShieldCheck size={13} /><span>Every listed entry and exit is a deterministic historical model under the selected next-bar-open execution and declared costs. It is not a broker order or recommendation.</span></div></>}</div>}
    {tab === "properties" && <div className="protocol-content backtest-evidence-panel"><div className="protocol-banner"><Play size={14} /><span><b>Deterministic historical evidence</b><small>Required: compiled source + verified candles. Signal at close; modeled market fill at next open.</small></span><StatusPill tone={backtest?.status === "COMPLETED" ? "ready" : "pending"}>{backtest?.classification.label ?? (compile.data?.ok ? "READY TO EVALUATE" : "SOURCE VALIDATION REQUIRED")}</StatusPill></div><section className="protocol-section"><div className="protocol-section-heading"><span>1</span><div><b>Execution and cost assumptions</b><small>Every cost remains explicit. No limit/stop simulation, optimization, forecast, or broker route is included.</small></div></div><div className="backtest-config-grid"><label>Initial capital<input type="number" min="1" value={backtestConfig.initialCapital} onChange={event => updateConfig("initialCapital", Number(event.target.value))} /></label><label>Quantity<input type="number" min="0.000001" step="any" value={backtestConfig.positionSize} onChange={event => updateConfig("positionSize", Number(event.target.value))} /></label><label>Multiplier<input type="number" min="0.000001" step="any" value={backtestConfig.multiplier} onChange={event => updateConfig("multiplier", Number(event.target.value))} /></label><label>Commission / unit<input type="number" min="0" step="any" value={backtestConfig.commissionPerUnit} onChange={event => updateConfig("commissionPerUnit", Number(event.target.value))} /></label><label>Spread ticks<input type="number" min="0" step="any" value={backtestConfig.spreadTicks} onChange={event => updateConfig("spreadTicks", Number(event.target.value))} /></label><label>Slippage ticks<input type="number" min="0" step="any" value={backtestConfig.slippageTicks} onChange={event => updateConfig("slippageTicks", Number(event.target.value))} /></label><label>Tick size<input type="number" min="0.00000001" step="any" value={backtestConfig.tickSize} onChange={event => updateConfig("tickSize", Number(event.target.value))} /></label></div><div className="backtest-run-actions"><button className="terminal-primary-button" onClick={runHistoricalEvidence} disabled={backtestRunning || !bars.length || !compile.data?.ok}>{backtestRunning ? <><LoaderCircle size={14} /> Preparing evidence</> : <><Play size={14} /> Run verified window</>}</button><label className="marker-toggle"><input type="checkbox" checked={showTradeMarkers} onChange={event => setShowTradeMarkers(event.target.checked)} /><span>Show entry / exit markers</span></label><small>{bars.length.toLocaleString("en-US")} verified chart bars · {compile.data?.ok ? "closed-source worker evaluation" : "compile source first"} · no cloud save</small></div></section>{backtestError && <div className="protocol-issues"><span><CircleAlert size={12} />{backtestError}</span></div>}{backtest && <><section className="protocol-section backtest-result-section"><div className="protocol-section-heading"><span>2</span><div><b>{backtest.status === "COMPLETED" ? "Reproducible evidence package" : "Evaluation unavailable"}</b><small>{backtest.runId} · {backtest.engineVersion} · {backtest.hash}</small></div></div><div className="backtest-classification"><StatusPill tone={backtest.classification.kind === "UNCLASSIFIED" ? "blocked" : "ready"}>{backtest.classification.label}</StatusPill><span>{backtest.classification.baselineFingerprint ?? (backtest.classification.kind === "DIRECT_SOURCE" ? "Closed source and declared costs retained" : "No immutable protocol fingerprint")}{backtest.classification.incrementField ? ` · changed ${backtest.classification.incrementField}` : ""}</span></div>{backtest.metrics ? <div className="backtest-metrics"><div><span>Net P&amp;L</span><b className={backtest.metrics.netPnl >= 0 ? "positive" : "negative"}>{backtest.metrics.netPnl >= 0 ? "+" : ""}{backtest.metrics.netPnl.toFixed(2)}</b></div><div><span>Return</span><b className={backtest.metrics.returnPct >= 0 ? "positive" : "negative"}>{backtest.metrics.returnPct >= 0 ? "+" : ""}{backtest.metrics.returnPct.toFixed(2)}%</b></div><div><span>Max DD</span><b>{backtest.metrics.maxDrawdown.toFixed(2)} · {backtest.metrics.maxDrawdownPct.toFixed(2)}%</b></div><div><span>Trades</span><b>{backtest.metrics.tradeCount}</b></div></div> : <div className="protocol-blockers"><b>Evaluation limitation</b><span>{backtest.limitations.at(-1)}</span></div>}<div className="backtest-provenance"><b>Verified provenance</b><span>{backtest.provenance.provider?.toUpperCase() ?? "PROVIDER UNSPECIFIED"} · {backtest.provenance.symbol ?? "SYMBOL UNSPECIFIED"} · {backtest.provenance.interval ?? "INTERVAL UNSPECIFIED"}</span><small>{backtest.provenance.normalizedBars.toLocaleString("en-US")} normalized bars · {backtest.provenance.coverageComplete ? "complete selected coverage" : "coverage not complete"} · {backtest.provenance.fingerprint ?? "no fingerprint"}</small><small>Source timestamp {backtest.provenance.sourceTimestamp ? new Date(backtest.provenance.sourceTimestamp).toISOString() : "unavailable"} · rejected {backtest.provenance.rejectedBars} · deduplicated {backtest.provenance.duplicateBars}</small></div></section>{backtest.metrics && <section className="protocol-section"><div className="protocol-section-heading"><span>3</span><div><b>Monthly outcomes and trades</b><small>Trade outcomes use the configured costs. Terminal marking is disclosed separately from next-open fills.</small></div></div><div className="monthly-outcomes">{backtest.monthlyOutcomes.map(outcome => <div key={outcome.month}><span>{outcome.month}</span><b className={outcome.returnPct >= 0 ? "positive" : "negative"}>{outcome.returnPct >= 0 ? "+" : ""}{outcome.returnPct.toFixed(2)}%</b><small>{outcome.pnl >= 0 ? "+" : ""}{outcome.pnl.toFixed(2)}</small></div>)}</div><div className="backtest-trades">{backtest.trades.slice(0, 12).map(trade => <div key={trade.id}><b>#{trade.id}</b><span>{new Date(trade.entryTime).toISOString().slice(0, 16).replace("T", " ")} → {new Date(trade.exitTime).toISOString().slice(0, 16).replace("T", " ")}</span><em className={trade.netPnl >= 0 ? "positive" : "negative"}>{trade.netPnl >= 0 ? "+" : ""}{trade.netPnl.toFixed(2)}</em><small>{trade.reason === "end_of_data_mark" ? "terminal mark" : "next-open exit"} · costs {trade.costs.toFixed(2)}</small></div>)}</div></section>}<div className="compiler-boundary"><ShieldCheck size={13} /><span>Historical research evidence only. It does not establish future performance, create orders, route to a broker, or evaluate arbitrary JavaScript source. Walk-forward, Monte Carlo, optimization, and historical tick/order-flow replay remain out of scope.</span></div></>}</div>}
  </aside>;
}
